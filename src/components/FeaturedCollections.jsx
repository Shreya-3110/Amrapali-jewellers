...
export default function FeaturedCollections() {
  const spotlights = [
    ...
    },
    {
      ...
      code: 'RUD-NEP-MALA',
      isOffset: true, // Elegant staggered center position
    },
    {
      id: '03',
      edition: '№ 03',
  ...
  return (
    <section id="collections" className="w-full py-24 sm:py-32 bg-[#FAF7F2] border-b border-amber-900/10 relative overflow-hidden">
      ...
      <div className="max-w-[1440px] mx-auto px-6 sm:px-10 lg:px-16 relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between border-b border-amber-900/15 pb-8 mb-16 gap-6">
          <div>
            ...
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-serif font-light text-[#1C1917] tracking-tight">
          ...
          <div className="text-left md:text-right max-w-md">
            ...
            <div className="flex items-center md:justify-end gap-2 text-[#8B5E14] text-[10px] tracking-[0.25em] font-serif uppercase mt-2.5">
              <span>✦</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 lg:gap-12 xl:gap-16 items-start">
          ...
          {spotlights.map((item) => (
            <div
              key={item.id}
              className={`bg-white/95 backdrop-blur-xl rounded-[2rem] p-6 sm:p-7 border border-amber-900/10 shadow-[0_15px_45px_-12px_rgba(28,25,23,0.06)] hover:shadow-[0_25px_60px_-10px_rgba(139,94,20,0.16)] hover:border-amber-400/40 hover:-translate-y-2 transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] group flex flex-col justify-between transform-gpu ${
                item.isOffset ? 'md:mt-12' : ''
              }`}
              style={{ transform: 'translate3d(0, 0, 0)', backfaceVisibility: 'hidden' }}
            >
              {/* Card Top Row: Edition & SKU */}
              <div className="flex items-center justify-between text-[11px] font-sans font-semibold text-[#8B5E14] tracking-[0.25em] uppercase border-b border-amber-900/10 pb-3.5 mb-5">
                <span>{item.edition} • {item.category}</span>
                <span className="text-stone-400 font-mono text-[10px] tracking-normal">{item.code}</span>
              </div>

              {/* Video Media Container with Soft Radius & Hardware Acceleration */}
              <div className="w-full aspect-[4/5] bg-stone-950 rounded-2xl overflow-hidden relative shadow-inner ring-1 ring-amber-900/10 group-hover:ring-amber-500/30 transition-all duration-700 transform-gpu">
                <video
                  src={item.videoSrc}
                  poster={item.poster}
                  autoPlay
                  ...
                  playsInline
                  preload="auto"
                  className="w-full h-full object-cover group-hover:scale-104 transition-transform duration-1000 ease-out transform-gpu"
                />
                ...
                {/* Bottom Origin Pill */}
                <div className="absolute bottom-3.5 left-3.5 right-3.5 flex items-center justify-between text-[10px] text-amber-100/95 font-sans bg-black/60 backdrop-blur-md px-3 py-1.5 rounded-xl border border-white/10 pointer-events-none">
              ...
              <div className="pt-6 space-y-3.5 flex-1 flex flex-col justify-between">
                <div className="space-y-2">
                  <h3 className="text-xl sm:text-2xl font-serif font-normal text-[#1C1917] group-hover:text-[#8B5E14] transition-colors duration-500 leading-snug">
                    {item.title}
                  </h3>
